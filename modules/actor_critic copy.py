...
class SimpleAttention(nn.Module):
    ...
    def forward(self, query, keys):
        # query: (B, D), keys: (B, 2, D)
        q = self.query_proj(query).unsqueeze(1)  # (B, 1, D)
        k = self.key_proj(keys)  # (B, 2, D)
        scores = torch.sum(q * k, dim=-1)  # (B, 2)
        weights = self.softmax(scores)  # (B, 2)
        return weights

class FusionPolicyWithCritic(nn.Module):
    def __init__(self, expert_wheel, expert_biped, main_encoder, obs_dim, latent_dim, action_dim, critic_hidden_dims=[256, 256]):
        super().__init__()
        self.expert_wheel = expert_wheel
        self.expert_biped = expert_biped
        self.main_encoder = main_encoder
        self.attention = SimpleAttention(latent_dim, hidden_dim=latent_dim)
        self.critic = self._build_mlp(obs_dim, 1, critic_hidden_dims)
        self.std = nn.Parameter(torch.ones(action_dim))
        self.distribution = None

    def _build_mlp(self, in_dim, out_dim, hidden_dims):
        layers = []
    ...
    def forward(self, obs, obs_hist):
        latent_wheel = self.expert_wheel.actor_teacher_backbone.mlp_encoder(obs_hist)
        latent_biped = self.expert_biped.actor_teacher_backbone.mlp_encoder(obs_hist)
        latent_main = self.main_encoder(obs_hist)

        latents = torch.stack([latent_wheel, latent_biped], dim=1)  # (B, 2, D)
        weights = self.attention(latent_main, latents)  # (B, 2)
    ...
    def act(self, obs, obs_hist):
        mean, _ = self.forward(obs, obs_hist)
