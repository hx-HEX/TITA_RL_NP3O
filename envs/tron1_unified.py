...
class Tron1UnifiedRobot(BaseTask):
    ...
    def step(self, actions, current_learning_iteration):
        ...
        self._update_reward_scales(self.current_learning_iteration)
        self.post_physics_step()
    ...
    def _update_reward_scales(self, step_idx):
        """根据训练进度动态更新各奖励项系数"""
        s = self.reward_scales  # 当前权重
        init = self.initial_scales
        if step_idx > 1500:
            alpha = min((step_idx - 1500) / 1000, 1.0)
            s["tracking_lin_vel"] = init["tracking_lin_vel"] * (1.0 + 1.0 * alpha)
            s["tracking_ang_vel"] = init["tracking_ang_vel"] * (1.0 + 1.0 * alpha)
            s["tracking_contacts_shaped_height_wheel"] = init["tracking_contacts_shaped_height_wheel"] * (1.0 - 1.0 * alpha)

        if step_idx > 20000:
            alpha = min((step_idx - 20000) / 1000, 1.0)
            # s["tracking_lin_vel"] = init["tracking_lin_vel"] * (1.0 + 1.0 * alpha)
            # s["tracking_ang_vel"] = init["tracking_ang_vel"] * (1.0 + 1.0 * alpha)
            s["tracking_contacts_shaped_force_wheel"] = init["tracking_contacts_shaped_force_wheel"] * (1.0 - alpha)
            s["tracking_contacts_shaped_vel_wheel"] = init["tracking_contacts_shaped_vel_wheel"] * (1.0 - alpha)
            # s["tracking_contacts_shaped_height_wheel"] = init["tracking_contacts_shaped_height_wheel"] * (1.0 - alpha)
            # s["phase_wheel_vel_wheel"] = init["phase_wheel_vel_wheel"] * (1.0 - alpha)
            s["simple_coordination_wheel"] = init["simple_coordination_wheel"] * (1.0 - alpha)
            s["no_fly_wheel"] = init["no_fly_wheel"] * (1.0 - alpha)
            s["same_foot_x_position"] = -alpha*5.0*self.dt
            s["same_foot_z_position"] = -alpha*10.0*self.dt
            s["leg_symmetry"] = -alpha*5.0*self.dt
    ...
    def compute_reward(self):
        ...
        for i in range(len(self.reward_functions)):
            ...
            current_scale = self.reward_scales.get(name, 0.0)
            ...
            try:
                ...
                self.rew_buf += rew
                self.episode_sums[name] += rew.detach()
                
            except Exception as e:
                print(f"计算奖励 {name} 时出错: {e}")
                # 继续处理其他奖励，避免单个奖励失败影响整体
    ...
    def reset(self):
        ...
        obs,_,_, _, _,_,_,_,= self.step(
            torch.zeros(self.num_envs, self.num_actions, device=self.device, requires_grad=False),self.current_learning_iteration)
        return obs
    ...
    def _prepare_reward_function(self):
        ...
        self.initial_scales = self.reward_scales.copy()
        # reward episode sums
    ...
    def _resample_commands(self, env_ids):
        ...
        # ---------- base height ----------
        rand_h = torch_rand_float(
            self.command_ranges["height"][0],
            self.command_ranges["height"][1],
            (len(env_ids), 1),
            device=self.device
        ).squeeze(1)
        self.commands[env_ids, 4] = rand_h

        # if self.current_learning_iteration < 10000:
        #     prob_foot = 1.0
        #     prob_jump = 0.0
        #     prob_spin = 0.0
        # else:
        #     beta = torch.min((self.current_learning_iteration - 10000)/5000, 1.0)
        #     prob_foot = 1.0 - 0.5*beta
        #     prob_jump = 0.3*beta   
        #     prob_spin = 0.3*beta
        ...
        prob_jump = 0.3 
        prob_spin = 0.3
        # ---------- binary modes (gait/jump/spin) ----------
        gait_rand = (torch.rand(len(env_ids), device=self.device) < prob_foot).float()  # 0/1 foot indicator
        self.commands[env_ids, 5] = gait_rand

        # sample jump flag with low probability
        jump_rand = (torch.rand(len(env_ids), device=self.device) < prob_jump).float()
        self.commands[env_ids, 6] = jump_rand

        # sample spin flag with low probability
        spin_rand = (torch.rand(len(env_ids), device=self.device) < prob_spin).float()
        self.commands[env_ids, 7] = spin_rand

        # ---------- enforce logical constraints / special handling ----------
        # If spin_flag active => force wheel mode and disable jump
        spin_envs = env_ids[spin_rand.nonzero(as_tuple=False).squeeze(1)] if spin_rand.sum() > 0 else torch.tensor([], dtype=torch.long, device=self.device)
        if spin_envs.numel() > 0:
            # For spin envs, override wz with high-speed sampling
            # spin_high_speed is in your config or set default
            spin_speed = getattr(self.cfg.commands, "spin_high_speed", 20.0)

            # 先采样符号（正或负）
            sign = torch.where(torch.rand(spin_envs.numel(), device=self.device) < 0.5,
                            torch.tensor(-1.0, device=self.device),
                            torch.tensor( 1.0, device=self.device))

            # 再采样绝对值范围 [10, spin_speed]
            min_speed = 10.0
            abs_val = torch_rand_float(min_speed, spin_speed,
                                        (spin_envs.numel(), 1), 
                                        device=self.device).squeeze(1)

            # 最终速度 = 符号 × 绝对值
            spin_wz = sign * abs_val

            self.commands[spin_envs,2] = spin_wz
   ...
   #------------ reward functions----------------
    ...
    def _reward_lin_vel_z(self):
        ...
        return torch.square(self.base_lin_vel[:, 2]) * (self.commands[:, 6] < 0.5)*(self.commands[:, 7] < 0.5)
    ...
    def _reward_feet_distance(self):
        ...
        return reward*(self.commands[:, 7] < 0.5)

    def _reward_base_height_tracking(self):
        # Penalize base height away from target
        base_height_target = self.commands[:,4]
        base_height = torch.mean(self.root_states[:, 2].unsqueeze(1) - self.measured_heights, dim=1)
        err = (base_height - base_height_target) ** 2
        sigma = getattr(self.cfg.rewards, "height_tracking_sigma", 0.05)
        return torch.exp(-err / (sigma + 1e-8))*(self.commands[:, 6] < 0.5)*(self.commands[:, 7] < 0.5)
    ...
    def _reward_tracking_lin_vel(self):
        ...
        return torch.exp(-lin_vel_error/self.cfg.rewards.tracking_sigma)*((self.commands[:, 7] < 0.5))*((self.commands[:, 5] < 0.5))
    
    def _reward_tracking_lin_vel_feet(self):
        # Tracking of linear velocity commands (xy axes)
        lin_vel_error = torch.sum(torch.square(self.commands[:, :2] - self.base_lin_vel[:, :2]), dim=1)
        return torch.exp(-lin_vel_error/self.cfg.rewards.tracking_sigma)*((self.commands[:, 7] < 0.5))*((self.commands[:, 5] > 0.5))
    ...
    def _reward_tracking_ang_vel(self):
        ...
        base_reward = torch.exp(-ang_vel_error/self.cfg.rewards.ang_tracking_sigma)
        spin_bonus = torch.exp(-(ang_vel_error)/(self.cfg.rewards.ang_tracking_sigma/4))
        return base_reward*(self.commands[:,7]<0.5)*((self.commands[:, 5] < 0.5)) + spin_bonus*(self.commands[:,7]>0.5)

    def _reward_tracking_ang_vel_feet(self):
        # Tracking of angular velocity commands (yaw) 
        ang_vel_error = torch.square(self.commands[:, 2] - self.base_ang_vel[:, 2])
        base_reward = torch.exp(-ang_vel_error/self.cfg.rewards.ang_tracking_sigma)
        return base_reward*((self.commands[:, 5] > 0.5))*(self.commands[:,7]<0.5) 
    ...
    def _reward_leg_symmetry(self):
        ...
        return torch.abs(leg_symmetry_err)*((self.commands[:, 5] < 0.5))*(self.commands[:, 7] < 0.5)
    ...
    def _reward_same_foot_x_position(self):
        ...
        vel_cmds = self.commands[:, :2]
        standing = torch.all(torch.abs(vel_cmds) < 0.1, dim=1) 
        return reward*( (self.commands[:, 5] < 0.5) | (standing & (self.commands[:, 5] > 0.5)) )*(self.commands[:, 7] < 0.5)
    ...
    def _reward_same_foot_z_position(self):
        ...
        return (foot_z_position_err ** 2)*((self.commands[:, 5] < 0.5))*(self.commands[:, 7] < 0.5)

    def _reward_no_fly_wheel(self):
        # 当前帧接触情况
        ...
        reward[no_contact_moving] = -1.0  # 飞行：严重惩罚
        ...
        return reward*(self.commands[:, 5] < 0.5)*(self.commands[:, 6] < 0.5)*(self.commands[:, 7] < 0.5)
    
    #步态相关奖励
    def _reward_tracking_contacts_shaped_force_wheel(self):
        ...
        standing = torch.all(torch.abs(vel_cmds) < 0.1, dim=1)
        moving = ~standing
        return torch.where(moving, reward / len(self.feet_indices), 0)*(self.commands[:, 5] < 0.5)*(self.commands[:, 6] < 0.5)*(self.commands[:, 7] < 0.5)
        # return reward / len(self.feet_indices)

    def _reward_tracking_contacts_shaped_vel_wheel(self):
    ...
    def _reward_tracking_contacts_shaped_height_wheel(self):
    ...
    def _reward_phase_wheel_vel_wheel(self):
        # 判断是否在移动（结合命令和实际速度）
        vel_cmds = self.commands[:, :2]
        ...
        for i in range(len(self.feet_indices)):
            swing_phase = (desired_contact[:, i] < 0.5).float()
            # 添加轮子运动惩罚：摆动期对应的轮子速度过高
            if i == 0:  # 左腿
                wheel_vel = torch.abs(self.dof_vel[:, 3])
            else:  # 右腿
                wheel_vel = torch.abs(self.dof_vel[:, 7])
                
            # 摆动期轮子速度不应过高（防止偷奖励）
            wheel_penalty = swing_phase * torch.clip(wheel_vel, 0, 1)
            reward += wheel_penalty
        return reward * moving*(self.commands[:, 5] < 0.5)*(self.commands[:, 6] < 0.5)*(self.commands[:, 7] < 0.5)

    def _reward_simple_coordination_wheel(self):
        """最简单的协调性检查"""
        if not hasattr(self, 'contact_balance_counter'):
            self.contact_balance_counter = torch.zeros((self.num_envs, 2), device=self.device)
        
        # 当前接触
        foot_forces_z = self.contact_forces[:, self.feet_indices, 2]
        left_contact = (foot_forces_z[:, 0] > 0.5)
        right_contact = (foot_forces_z[:, 1] > 0.5)
        
        # 累计接触时间
        self.contact_balance_counter[:, 0] += left_contact.float()
        self.contact_balance_counter[:, 1] += right_contact.float()
        
        penalty = torch.zeros(self.num_envs, device=self.device)
        
        # 每50步检查一次平衡性
        if hasattr(self, 'global_counter') and (self.global_counter % 100 == 0):
            total = self.contact_balance_counter.sum(dim=1)
            left_ratio = self.contact_balance_counter[:, 0] / (total + 1e-6)
            
            # 理想比例是0.5（双腿接触时间相等）
            imbalance = torch.abs(left_ratio - 0.5)
            # print("imbalance",imbalance)
            penalty = torch.sigmoid(imbalance * 10) * 0.3  # 差异越大惩罚越重
            
            # 重置计数器
            self.contact_balance_counter.zero_()
        
        return penalty*(self.commands[:, 5] < 0.5)*(self.commands[:, 6] < 0.5)*(self.commands[:, 7] < 0.5)
    ...
    def _reward_phase_wheel_vel(self):
        # 判断是否在移动（结合命令和实际速度）
        vel_cmds = self.commands[:, :2]
    ...
    def _reward_simple_coordination(self):
        """最简单的协调性检查"""
        if not hasattr(self, 'contact_balance_counter'):
            self.contact_balance_counter = torch.zeros((self.num_envs, 2), device=self.device)
    ...
    def _reward_fly(self):
        ...
        return reward*(self.commands[:, 6] > 0.5)

    def _reward_jump_landing(self):
        forces = torch.norm(self.contact_forces[:, self.feet_indices, :], dim=-1)
        landing_force = torch.sum(forces, dim=1)
        return torch.exp(- landing_force * 0.02) * (self.commands[:, 6] > 0.5)
